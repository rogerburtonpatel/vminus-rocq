...
Proof.
  ...
  - eapply t_succ; eauto. 
  - eapply t_nrec; eauto. 
    eapply IHh3. eapply typing_subst_lift. auto. 
  - eapply t_let; eauto. 
    eapply IHh2. eapply typing_subst_lift. auto. 
Qed. 
...
Proof.
  ...
  - apply t_lit.
  (* succ *)
  - apply t_succ. apply IHhS. apply H0.    
  (* nrec_base *)
  - apply H4.
  (* app with nrec *)
  - eapply t_app. eapply substitution; eauto.  
    eapply typing_subst_cons. apply t_lit.
    eapply typing_subst_id.
    eapply t_nrec; eauto.
    eapply t_lit.
  (* nrec *)
  - eapply t_nrec; eauto. 
...
Proof.
  ...
  + destruct (IHhT e0); auto.
    ++ right. destruct (canonical_forms_Nat e0 hT H).
      rewrite H0. exists (lit (S x)). apply Small.s_succ_lit.
    ++ right. destruct H. exists (succ x). apply Small.s_succ_cong. apply H. 
  + destruct (IHhT1 e2); auto.
    ++ right.
       eapply canonical_forms_Nat in H; eauto.
       destruct H as [[|e2''] ->]. (* neat trick. unfortunately it is illegible. *) 
       exists e0. apply Small.s_nrec_zero.
       exists (app (e1 [(lit e2'') ..]) (nrec (lit e2'') e0 e1)).
       apply Small.s_nrec_succ.
    ++ right.
      destruct H as [e2' h1].
      exists (nrec e2' e0 e1).
      apply Small.s_nrec_cong; auto. 
  + destruct (IHhT1 e1); auto.
    ++ right. 
       exists (e2[e1 ..]).
       apply Small.s_letv; eauto.
    ++ right. 
       destruct H as [e1' h1].
       exists (let_ e1' e2).
       apply Small.s_let_cong; auto. 
Qed.   
...
(* Example succ_steps : Small.step (succ (lit 0)) (lit 1). 
Proof. 
  eapply Small.s_succ_lit.
Qed. 

Example lit_safe : safe_run Small.step (lit 1).
Proof. 
  eapply safe_val. done. 
Qed. 


Definition zero_to_1 : safe_run Small.step (succ (lit 0)) := 
  safe_step Small.step (succ (lit 0)) (lit 1)
  succ_steps lit_safe.
...
CoFixpoint all_1s : colist nat := cocons nat 1 all_1s.

Fixpoint take (c : colist nat) (n : nat) := 
  match n, c with 
  | 0, _ => nil
  | S n', cocons _ x xs => cons x (take xs n')
  end. 

CoInductive X : Type. 
Fail CoFixpoint foo (x : X) : X := foo x. 

  
Example foo : (take all_1s 100) <> nil.
Proof. 
  simpl. (* to show what take all_1s 100 looks like. *)
  easy. 
Qed. 
CoInductive Infinite A : colist A -> Prop :=
  | Inf : forall x l, Infinite A l -> Infinite A (cocons A x l).

Example all_1s_infinite : Infinite nat all_1s.
Proof. 
  cofix CH.   
  inversion CH.  
  apply Inf.
